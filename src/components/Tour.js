...
const Tour = ({ value }) => {
    ...
    return (
        <div className="Tour" style={{ width: '20vw', height: '80vh' }}>
            ...
            {showmore ? <button id={`see-more-${value.id}`} onClick={() => setShowmore(false)}>Show more</button> : <button id={`delete-btn-${value.id}`} onClick={() => setShowmore(true)}>Show less</button>}
